...
var App = new Vue({
    ...
    methods: {
        ...
        getCard: function getCard(id) {
            if (this.currentType == 1) {
                ...
                $.ajax({
                    url: "https://isapi.zujiekeji.cn/index/coupon/receiveZujieCoupon/?source=3",
                    type: "POST",
                    data: {
                        userId: userInfo.id,
        ...
        getStoreCard: function getStoreCard(storeid, id) {
            if (this.currentType == 1) {
                ...
                userInfo = JSON.parse(userInfo);
                $.ajax({
                    url: "https://isapi.zujiekeji.cn/index/coupon/receiveStoreCoupon",
                    type: "POST",
                    data: {
                        storeId: storeid,
                        userId: userInfo.id,
    ...
    mounted: function mounted() {
        ...
        $.ajax({
            url: "https://isapi.zujiekeji.cn/index/store/getActivityStore",
            type: "GET",
            data: {
                "act_id": 16
            },
